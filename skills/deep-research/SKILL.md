...
> **🔗 SUPERSEDED BY `master-deep-research`** — The master skill merges this skill's CRAAP-lite tiering + confidence calibration with the mimic-Perplexity parallel swarm architecture, adds triple-weighted scoring (relevance × recency × tier), and includes a size-driven auto-expansion loop (`--min-kb` / `--min-words`). Load `master-deep-research` for the full pipeline. This skill remains valid for simpler single-agent research tasks or when swarm overhead isn't warranted.

> The user provides the research topic directly. Optionally append `--depth=shallow` for a quick 5-source pass; default is **deep** mode (30–50 source comprehensive report).

## Overview

This skill ingests 30–50 sources in a single session, but unlike most "deep research" pipelines it does not weight every URL equally. Each source is classified by type (primary / secondary / tertiary) and scored on a CRAAP-lite rubric (Authority, Recency, Verifiability) producing a tier (T1 / T2 / T3). Every finding in the final report carries an explicit confidence level grounded in how many T1 sources corroborate it. The report includes a "Falsifiable claims" section so the reader knows what evidence would change the conclusion.

Run on-demand when the user asks for deep research on a topic. Not recommended for trivia or quick lookups — save it for questions that warrant the depth.
...
Extract topic and depth from the user's request:
- If the request contains `--depth=shallow`, use shallow mode (5 sources, ~600 words).
- Otherwise default to **deep** mode (30–50 sources, 3,000–5,000 words).
- The research topic is everything before any `--depth=` flag.
- Example: `"AI agent security 2026 --depth=deep"` → topic = "AI agent security 2026", depth = deep.

If `~/.hermes/memories/MEMORY.md` exists, read it for prior research context, tracked interests, and related findings.

---

### 1. Landscape search (all depths)

Run **5–8 distinct web searches** to map the topic space using `web_search`:
...
Collect URLs from results. Filter out paywalled content (URLs containing `/paywall`, `subscribe`, `sign-in`) and obvious low-quality aggregators. Deduplicate by canonical domain+path. Target ≥30 unique sources for deep mode.
...
If rate-limited (429), wait 5 seconds and retry once. If still failing, fall back to `web_search` on `semanticscholar.org` with the topic.
...
Select **top 10** papers by relevance × citation × recency. Tier-1 papers get full abstract fetch (or first 3,000 words of open-access PDF via `web_extract`); tier-2 use abstract from API only.

---

### 3. Full content ingestion

**Shallow mode:** Fetch 5 URLs in full with `web_extract`.
**Deep mode:** Fetch top **30 URLs** with `web_extract`.
...
Aim for **deep mode**: at least 8 T1, at least 12 T2, no more than 5 T3 in the cited set. If the mix is worse than this, run 2–3 supplementary web searches targeting authoritative sources (".gov", ".edu", "site:arxiv.org", official org names) before writing.
...
Save to `~/.hermes/articles/deep-research-${today}.md`.
...
Send via `send_message` to Slack:
...
Full report: ~/.hermes/articles/deep-research-${today}.md
```

If `send_message` is unavailable, log `DEEP_RESEARCH_NOTIFY_FAILED` and continue — the article file is the authoritative record.
...
- **Tool mapping:** Use `web_search` for landscape searches (step 1), `web_extract` for fetching full page content (step 3), and `send_message` for Slack notifications (step 7). Semantic Scholar and arXiv use `curl` via `terminal`.
