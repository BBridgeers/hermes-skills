...
description: Weekly ranking of which skills are most popular across CONFIGURED Aeon forks — external ecosystem monitoring. Tracks Aeon FORK skill adoption (not Hermes skills) to surface trends, signal promote/match/sunset opportunities, and monitor what the broader Aeon ecosystem is building that Hermes could adopt.
tags: [meta, monitoring]
---

# Skill Leaderboard — Aeon Fork Ecosystem Intelligence

> Adapted from Aeon's skill-leaderboard. This skill tracks **Aeon FORK skill popularity — NOT Hermes' own skills.** Hermes uses this to monitor what skills people are building and running in the Aeon ecosystem that we could copy or learn from. Core methodology: configured-fork denominator, tiered fleet classification, promote/match/sunset heuristics, and week-over-week delta tracking — all preserved. Adapted for Hermes: replaces `./notify` with Slack `send_message`, state/log paths use `~/.hermes/` hierarchy, target repo pulled from `~/.hermes/memories/watched-repos.md`, article path → `~/.hermes/articles/skill-leaderboard-DATE.md`.

## Why this version

Upstream Aeon's `aeon.yml` ships with effectively only `heartbeat: enabled: true`. Every fresh fork inherits that default. If we score "skills enabled" across **all active forks**, the leaderboard is a tautology (heartbeat always wins, everything else hovers near 0) and the operator learns nothing. The lever is to score against **configured forks only** — forks whose `aeon.yml` diverges from upstream defaults — and to convert the result into three actionable recommendations: which skills to promote, which fleet patterns to copy upstream, which skills to sunset.
...
```bash
mkdir -p ~/.hermes/articles ~/.hermes/state ~/.hermes/logs
gh api user --jq '.login' 2>/dev/null || echo "GH_AUTH_FAILED"
```

- If `GH_AUTH_FAILED` is printed: log `SKILL_LEADERBOARD_NO_GH_AUTH` to `~/.hermes/logs/${date}.md` and stop. `gh` must be authenticated (`gh auth login`) or `GITHUB_TOKEN` must be set in the environment.
- Create the output directories if they don't exist (idempotent `mkdir -p`).

### 1. Determine the target repo

Read `~/.hermes/memories/watched-repos.md`. Use the first entry under a `## Aeon Forks` heading (or the first non-comment, non-empty line if unheaded). The format is `owner/repo`. If the file is missing, empty, or has no valid repo entry, log `SKILL_LEADERBOARD_NO_TARGET` to `~/.hermes/logs/${date}.md` and stop (no notification).

**If this is a first run** (file missing), create a stub `watched-repos.md` with the expected heading and a commented-out example (`# NousResearch/hermes-agent`) so the operator knows the format. Do NOT guess or hardcode an active repo — the operator must opt in.

### 2. Snapshot upstream defaults

Read Aeon's upstream `aeon.yml` from the main branch of the target repo:

```bash
gh api "repos/${TARGET_REPO}/contents/aeon.yml" --jq '.content' | base64 -d
```

Build `UPSTREAM_DEFAULTS`: a dict `{skill_name -> {enabled, model_or_null, var_or_empty, schedule_or_null}}` covering every skill entry under the `skills:` block. Also build `UPSTREAM_SKILLS`: the set of skill directory names from `skills/` (fetch via `gh api "repos/${TARGET_REPO}/contents/skills" --jq '.[].name'`). These are the comparison baselines.
...
If zero active forks: log `SKILL_LEADERBOARD_NO_FORKS` to `~/.hermes/logs/${date}.md` and stop (no notification).
...
Read `~/.hermes/state/skill-leaderboard-state.json` if it exists. Schema:
...
- **Fleet-only skills**: any `is_fork_only: true` skill enabled in ≥1 fork. Surface for review — the fleet built something upstream doesn't have. These are potential candidates for Hermes to adopt.

### 9. Write the article

To `~/.hermes/articles/skill-leaderboard-${date}.md`:

```markdown
# Skill Leaderboard — ${date}
...
---
*Source: GitHub API — forks of ${TARGET_REPO}. Methodology: a fork counts as "configured" if its `aeon.yml` differs from upstream defaults on `enabled`, `model`, `var`, or `schedule` for any skill. Untouched templates are excluded from leaderboard math. Hermes tracks this ecosystem externally — the leaderboard reflects Aeon fork adoption, not Hermes skill usage.*
```
...
Full report: ~/.hermes/articles/skill-leaderboard-${date}.md
```

Notification is sent only when `N_CONFIGURED >= 2` (gated in step 6). Otherwise the run is silent.

If `send_message` is unavailable, log `SKILL_LEADERBOARD_NOTIFY_FAILED` to the daily log and continue — the article and state file are the authoritative records.

### 12. Persist the snapshot

Write `~/.hermes/state/skill-leaderboard-state.json`:
...
## Prerequisites (must be verified before step 1)

- **GitHub API auth**: `gh` CLI must be authenticated (`gh auth status` returns a logged-in user) OR `GITHUB_TOKEN` env var must be set. If neither is true, every `gh api` call will 401 and the entire pipeline fails silently. Check with `gh api user --jq '.login'` before starting. If it fails, log `SKILL_LEADERBOARD_NO_GH_AUTH` and stop.
- **Target repo configured**: `~/.hermes/memories/watched-repos.md` must exist with a `## Aeon Forks` heading and at least one uncommented `owner/repo` line. If missing, the cron job should have created it during a prior run or the operator must add it manually. The skill stops with `SKILL_LEADERBOARD_NO_TARGET` if absent.
- **Directory structure**: `~/.hermes/articles/`, `~/.hermes/state/`, `~/.hermes/logs/` must exist. Create them with `mkdir -p` at the start of each run if missing.
...
- Do not parse last week's article for week-over-week — use `~/.hermes/state/skill-leaderboard-state.json` only.
...
- All GitHub API calls use `gh api` which handles auth internally — **but only if `gh` is actually authenticated**. Verify auth before starting. If `gh api` returns 403 with `X-RateLimit-Remaining: 0`, back off 60s and retry once; on continued failure, record `status: "rate_limited"` for that fork and proceed with partial fleet (the verdict and source-status footer surface the gap).
- This skill uses only existing Hermes tools: `terminal()`, `read_file`, `write_file`, `search_files`, `send_message`. No new env vars or secrets required beyond `GITHUB_TOKEN` (or `gh auth login`).

## Note

This skill monitors the **Aeon ecosystem** — not Hermes itself. The leaderboard tells us what skills Aeon fork operators are building and running. Fleet-only skills are prime candidates for Hermes to evaluate and potentially adopt. The promote/match/sunset signals help Hermes stay aligned with where the Aeon community is investing effort, but the ultimate decision to adopt any pattern remains with the Hermes operator.
