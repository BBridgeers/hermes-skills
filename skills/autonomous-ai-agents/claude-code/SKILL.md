...
## Ollama Proxy Mode (Use Any Model)

Claude Code can run through Ollama's Anthropic-compatible API, enabling any model Ollama supports to power Claude Code sessions. No Anthropic API key needed. Two paths:

### Path A: Local Ollama Proxy (recommended when Ollama is signed in)
...
Quick launch: `ollama launch claude` or `ollama launch claude --model kimi-k2.6:cloud`

### Path B: Ollama Cloud Direct (API key — no Ed25519 registration needed)

When the local Ollama instance isn't signed in (headless VPS with no browser), use the ollama.com API directly:
...
Then: `claude --model qwen3-coder:480b -p "task"`

**Note:** Claude Code refuses to run as root. On VPS, create a non-root user or use other agents (Codex, Pi, OpenCode) which don't have this restriction.

### Headless via Ollama

```
ollama launch claude --model kimi-k2.6:cloud --yes -- -p "refactor the auth module"
```

The `--yes` flag auto-pulls the model, skips selectors, and requires `--model`. Arguments after `--` pass directly to Claude Code.

### Recommended Cloud Models (Ollama Cloud — May 2026)

**Coding-focused (highest priority):**
- `qwen3-coder:480b` — dedicated coding model, 510B params
- `kimi-k2.6` — multimodal agentic, swarm orchestration
- `glm-5.1` — SOTA on SWE-Bench Pro, agentic engineering
- `gpt-oss:120b` — OpenAI's open model via Ollama Cloud
- `deepseek-v4-pro` — 1M context, 3 reasoning modes
- `deepseek-v4-flash` — 284B MoE, 13B active, efficient reasoning
- `minimax-m2.7` — fast coding and professional productivity

**Additional cloud models:** `qwen3.5` (397B), `gemma4:31b`, `nemotron-3-super`, `qwen3-coder-next`, `gemini-3-flash-preview`, `deepseek-v3.2`, `ministral-3`, `devstral-small-2`, `qwen3-next`, `rnj-1`

Full cloud model list: `curl -s "https://ollama.com/search?c=cloud"` — also catalogued at `~/.hermes/skills/autonomous-ai-agents/pi/references/ollama-cloud-models.md`
