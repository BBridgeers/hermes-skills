...
# scan.sh — Security scanner for Hermes SKILL.md files
#
# Usage:
#   ~/.hermes/skills/skill-security-scan/scan.sh <path-to-SKILL.md>
#   ~/.hermes/skills/skill-security-scan/scan.sh --all              # Scan all skills
#   ~/.hermes/skills/skill-security-scan/scan.sh --all --json        # JSON output
#
# Exit codes:
...
if [[ "$SCAN_ALL" == "true" ]]; then
  while IFS= read -r f; do
    FILES+=("$f")
  done < <(find "$HERMES_SKILLS_DIR" -maxdepth 3 -name "SKILL.md" -type f 2>/dev/null | sort)
fi
