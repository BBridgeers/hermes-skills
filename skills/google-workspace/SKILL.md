...
- `references/rclone-headless-auth.md` — rclone Google Drive auth on headless VPS — OAuth code exchange pitfalls, correct approaches
...
## rclone Headless Auth (VPS)

When the user wants to mount Google Drive via rclone on a headless VPS, **never** use interactive `rclone config create` — it will timeout. See `references/rclone-headless-auth.md` for the full pattern.

Quick path:
1. Have the user run `rclone authorize drive` on their **laptop** (has browser)
2. They paste the JSON blob — it contains `refresh_token` (what makes auth persistent)
3. Write it directly to `/root/.config/rclone/rclone.conf` with `client_id`/`client_secret` from `/root/.hermes/google_token.json`
4. Verify with `rclone lsf gdrive_personal:`

OAuth authorization codes from browser URLs (`4/0AeoWuM8...`) are NOT usable — they expire in minutes and lack refresh tokens. Always ask for the `rclone authorize` output instead.
