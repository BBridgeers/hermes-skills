...
- **LinkedIn message monitoring** — see `references/linkedin-email-monitoring.md` for the email-forwarding pattern to catch recruiter messages in real-time
- **Multi-source CSV enrichment & scoring** — see `references/multi-source-csv-enrichment.md` for cross-referencing, deduplicating, and scoring leads from multiple source files into a single master CSV
